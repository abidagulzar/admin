...
if ( is_search() ) {
	...
	if ( $baxel_show_filter_word ) { $baxel_pageHeader = baxel_translation( '_SearchResults' ) . $baxel_archiveSep; }
	$baxel_pageHeader .= esc_html( get_search_query( false ) );
	
} else if ( is_category() ) {
	
	$temp_title = '';
	if ( $baxel_show_filter_word ) { $temp_title = baxel_translation( '_Category' ) . $baxel_archiveSep; }
	$baxel_pageHeader = single_cat_title( $temp_title, false );
			
} else if ( is_tag() ) {
	
	$temp_title = '';
	if ( $baxel_show_filter_word ) { $temp_title = baxel_translation( '_Tag' ) . $baxel_archiveSep; }
	$baxel_pageHeader = single_tag_title( $temp_title, false );
	
} else if ( is_author() ) {
	
	the_post();
	if ( $baxel_show_filter_word ) { $baxel_pageHeader = baxel_translation( '_Author' ) . $baxel_archiveSep; }
	$baxel_pageHeader .= get_the_author();
	rewind_posts();
	
} else if ( is_day() ) {
	
	if ( $baxel_show_filter_word ) { $baxel_pageHeader = baxel_translation( '_Archives' ) . $baxel_archiveSep; }
