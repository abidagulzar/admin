...
if ( !function_exists( 'baxel_customizer' ) ) {
	function baxel_customizer( $wp_customize ) {
		...
		/* Sanitizers */
		function baxel_sanitize_itself( $input ) {			
			return $input;
		}
		...
		$wp_customize->add_setting( 'baxel_font_primary', array( 'default' => 'Raleway', 'sanitize_callback' => 'baxel_sanitize_itself' ) );
		$wp_customize->add_setting( 'baxel_font_secondary', array( 'default' => 'Raleway', 'sanitize_callback' => 'baxel_sanitize_itself' ) );
		...
		$wp_customize->add_setting( 'baxel_font_logotype', array( 'default' => 'Ubuntu_Condensed', 'sanitize_callback' => 'baxel_sanitize_itself' ) );
		$wp_customize->add_setting( 'baxel_font_size_logo', array( 'default' => 50, 'sanitize_callback' => 'baxel_sanitize_integer' ) );
		$wp_customize->add_setting( 'baxel_logo_image', array( 'sanitize_callback' => 'baxel_sanitize_itself' ) );
		...
		$wp_customize->add_setting( 'baxel_logo_image_sticky', array( 'sanitize_callback' => 'baxel_sanitize_itself' ) );		
		...
		/* Footer */
		$wp_customize->add_setting( 'baxel_copyright_text', array( 'default' => '2017 Baxel. All rights reserved.', 'sanitize_callback' => 'baxel_sanitize_let_html' ) );
		...
		foreach ( baxel_get_terms() as $key => $val ) {
			...
			$wp_customize->add_control( $the_term, array(
				'label' => $label,
				'description' => esc_html__( 'Original:', 'baxel' ) . ' "' . $val . '"',
				'section' => 'baxel_section_translation',
				'settings' => $the_term,
				'type' => 'text',
			) );
