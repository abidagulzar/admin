...
<?php if ( !is_single() && baxel_apply_columns() ) { echo '<div class="' . baxel_apply_columns() . '">'; } ?>
	<article <?php post_class( 'clearfix' ); if ( !is_single() && !baxel_check_style_z() && $baxel_min_post_height ) { echo 'style="min-height: ' . $baxel_min_post_height . 'px;"'; } ?>>
    	<?php if ( baxel_check_style_z() ) { ?>
            <a class="post-styleZ clearfix" href="<?php the_permalink(); ?>">
                ...
                <div class="post-styleZ-inner<?php if ( has_post_thumbnail() ) { echo '-with-t'; } ?>">
                	<div class="table-cell-middle">
                        ...
                        <h1 class="post-styleZ-title"><?php the_title(); ?></h1>
                    </div>
                </div>
            </a>        
        <?php } else { ?>
            ...
            <div class="article-content-outer<?php echo baxel_apply_layout(); ?>">
                <div class="article-content-inner">
                    ...
                    <?php if ( $baxel_show_date ) { ?>
                        <div class="article-date-outer"><?php if ( is_single() ) { ?><span class="article-date"><?php echo get_the_date(); ?></span><?php } else { ?><a class="article-date" href="<?php the_permalink(); ?>"><?php echo get_the_date(); ?></a><?php } ?></div>
                    <?php } ?>
                    <h1 class="article-title"><?php if ( is_single() ) { ?><?php the_title(); ?><?php } else { ?><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a><?php } ?></h1>
